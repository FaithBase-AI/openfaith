...
type Columns<TTable extends Drizzle.Table> = TTable['_']['columns'] extends infer TColumns extends
  Record<string, Drizzle.Column<any>>
  ? TColumns
  : never
  ...
  | ((
      s: {
        ...
        >
          ? Schema.Schema<
              Exclude<
                PropertySignatureEncoded<InsertColumnPropertySignatures<TTable>[S]>,
                undefined | null
              >,
              Exclude<
    ...
    >
      ? Schema.Schema<
          Exclude<
            PropertySignatureEncoded<InsertColumnPropertySignatures<TTable>[Col]>,
            undefined | null
          >,
          any
        >
...
const literalSchema = Schema.Union(Schema.String, Schema.Number, Schema.Boolean, Schema.Null)
...
type PropertySignatureReplaceType<S, ReplaceWith> = S extends Schema.PropertySignature<
  infer TokenType,
  any,
  infer Name,
  infer TokenEncoded,
  ...
  infer R
>
  ? Schema.PropertySignature<TokenType, ReplaceWith, Name, TokenEncoded, Encoded, HasDefault, R>
  : never
...
> {
  ...
  for (const [name, column] of columnEntries) {
    if (!column.notNull) {
      schemaEntries[name] = Schema.optional(Schema.NullOr(schemaEntries[name]!)) as any
